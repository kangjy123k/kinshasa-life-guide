...
import { FwodRecorderSheet } from "@/components/FwodRecorderSheet";
import { FwodPronunciationsPopup } from "@/components/FwodPronunciationsPopup";
...
export default function FrenchWordClient({
  initialEntries,
}: {
  initialEntries: FrenchDailyEntry[];
}) {
  const [dynamicEntries, setDynamicEntries] =
    useState<FrenchDailyEntry[]>(initialEntries);
  ...
  // 服务端已合并 BANK + 动态条目，首屏直接落到今日，避免 4/22 闪一下再跳的回退
  const [index, setIndex] = useState(() =>
    todayIndex(mergeEntries(BANK, initialEntries)),
  );
  ...
  const [myStats, setMyStats] = useState<{ wordCount: number; exampleCount: number }>(
    { wordCount: 0, exampleCount: 0 },
  );
  ...
  // 拉取个人跟读统计（已跟读 X 个词 · X 例句）
  useEffect(() => {
    let cancelled = false;
    fetch("/api/fwod/recording/my-stats", { cache: "no-store" })
      .then((r) => r.json())
      .then((d: { ok?: boolean; wordCount?: number; exampleCount?: number }) => {
        if (cancelled) return;
        if (d?.ok) {
          setMyStats({
            wordCount: d.wordCount ?? 0,
            exampleCount: d.exampleCount ?? 0,
          });
        }
      })
      .catch(() => {});
  ...
  const refreshStats = useCallback(() => setStatsBump((n) => n + 1), []);

  // 服务端已经把动态词条注水进首屏；只有当服务端拉空（DB 抖动）时才再 fetch 兜底
  useEffect(() => {
  ...
  return (
    ...
    >
      <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-rose-100">
        ...
        {/* 跟读状态条：突出"我开口说法语"，连续天数收成小芯片 */}
        <div className="max-w-2xl mx-auto px-4 pb-2">
          <div className="flex items-center gap-2">
            <div className="flex-1 inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-[12px] font-semibold">
              <Mic size={13} />
              已跟读 {myStats.wordCount} 个词 · {myStats.exampleCount} 个例句
            </div>
            {progress.streak > 0 && (
      ...
      <main className="max-w-2xl mx-auto px-4 pt-4">
        ...
        {/* 主卡片 */}
        <WordCard key={entry.word} entry={entry} onChanged={refreshStats} />
...
function RecorderRow({
  ...
  // 拉本词条 / target 的录音条数（含自己的私有 / 提审）
  useEffect(() => {
    let cancelled = false;
    fetch(
      `/api/fwod/recording/list?date=${encodeURIComponent(entry.date)}&target=${target}`,
      { cache: "no-store" },
    )
      .then((r) => r.json())
      .then((d: { ok?: boolean; items?: Array<unknown> }) => {
        if (cancelled) return;
        if (d?.ok && Array.isArray(d.items)) setCount(d.items.length);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
    };
  }, [entry.date, target, bump]);
  ...
  const pillBase =
    "shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-medium transition-colors active:scale-95 touch-manipulation";
  const listenCls =
    tone === "onDark"
      ? `${pillBase} bg-white/15 text-white hover:bg-white/25`
      : `${pillBase} bg-emerald-100 text-emerald-700 hover:bg-emerald-200`;
  const recCls =
    tone === "onDark"
      ? `${pillBase} bg-white text-rose-600`
      : `${pillBase} bg-rose-500 text-white`;
